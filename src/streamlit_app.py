...
if select == 'Check for Keywords':
    if url_input: #once the button is clicked on "Submit Url"
        with st.spinner(text='Processing Text...'):
            with col_1:
                posting_title, posting_time, about_the_job = web_scraper.ln_job_scraper(job_url)
                #Getting the text from the "About the job section"
                keywords = text_processing.keyword_rank(about_the_job)
            ...
            with col_2:
                ...
                resume_keywords = text_processing.keyword_rank(resume_text)
...
if select == 'Compare Resume Similarity':
    if url_input:
        with st.spinner(text='Processing Text...', ):
            ...
            st.write(f'Posted {posting_time}')
            with col_1:
                similarity_score = text_processing.compare_resume(resume_text, about_the_job)
